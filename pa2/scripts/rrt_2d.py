...
class RRTStar2D(GoalBiasedGreedySteerKNeighborhoodRRTStarBase):
    ...
    def collision_free(self, c1, c2, step_size):
        ...
        # if c1 or c2 does not pass collision check, return false
        ...
        if self.allclose(c1, c2) == False:
            ...
            # take the first step
            ct = np.add(c1, np.multiply(l, np.subtract(c2,c1)))
            # keep steering until collision
            while self.valid(ct):
                # if we are too close to c2, check c2 and return its validity
                if self.distance(ct, c2) < step_size:
                    valid_c = self.valid(c2)
                    return valid_c
                ct = np.add(ct, np.multiply(l, np.subtract(c2,ct)))
            return False
        else:
            return True
    ...
    def steer(self, c0, c, step_size):
        ...
        # if they are too close, return c if valid
        ...
        prev = c0
        # take first step
        ct = np.add(c0, np.multiply(l, np.subtract(c,c0)))
        # keep steering until colision
        while self.valid(ct):
            # if too close check c
            if self.distance(ct, c) < step_size:
                ...
                if valid_c:
                    ...
                    return None
            prev = ct
            ct = np.add(ct, np.multiply(l, np.subtract(c,ct)))
